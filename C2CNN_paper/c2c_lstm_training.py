...
def load_and_preprocess_input2(file_path):
    ...
    for row_number in range(len(data_output) - 1):
        ...
        # Check if the absolute difference between y values is greater than 0.0012
        if abs(y_i - y_j) > :
            y_max = y_i
            y_min = y_j
...
if __name__ == "__main__":
    ...
    for data_scale in range(6, 7):
        ...
        # (batch, seq_len=100, features=slice*2+velocity_slice)
        X_train_scaled = X_train.reshape(i, 100, slice * 2 + velocity_slice).astype(np.float32)
        print("X train shape3:", X_train_scaled.shape, X_train_scaled)

        # (batch, seq_len=100, 1)
        y_train = y_train.reshape(i, 100, 1).astype(np.float32)
        ...
        for loop in (0, 6):
            ...
            input_shape = X_train.shape[1:]  # (100, slice*2+velocity_slice)
            print(input_shape)
            num_points = input_shape[0]  # 100
            print(num_points)
            ...
            class PointNetRegressionModel(nn.Module):
                """
                LSTM-based sequence regression:
                Input : (B, 100, F)
                Output: (B, 100)
                """
                def __init__(self, input_shape, num_points, dropout_rate=0.5,
                    ...
                    seq_len, feat_dim = input_shape  # (100, F)
                ...
                def forward(self, x):
                    # x: (B, 100, F)
                    out, _ = self.lstm(x)                  # (B, 100, H * D)
                    out = self.dropout(out)
                    out = self.head(out)                   # (B, 100, 1)
                    out = out.squeeze(-1)                  # (B, 100)
                    return out
            # ========================================================================

            logger.info("LSTM model defined.")
            ...
            model = PointNetRegressionModel(
                input_shape=(100, slice * 2 + velocity_slice),
                num_points=100,
                dropout_rate=0.5,
                hidden_size=128,
                num_layers=2,
            ...
            def train_model(epochs, train_loader):
                ...
                for epoch in range(epochs):
                    for data, targets in train_loader:
                        ...
                        targets = targets.squeeze(-1)  # (B, 100)

                        optimizer.zero_grad()
                        outputs = model(data)          # (B, 100)
